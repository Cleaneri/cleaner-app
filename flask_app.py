...
def login():
    ...
    if request.method == "POST":
        ...
        if username in users:
            ...
            session["username"] = username
            session["admin"] = False
            session["bonus"] = True  # 🎁 İlk giriş bonusu
            return redirect("/dashboard")
...
def dashboard():
    ...
    if session.get("admin"):
        ...
        bonus = session.pop("bonus", False)
        return render_template("user_dashboard.html", cl=users[username]["cl"], messages=user_messages, bonus=bonus)
